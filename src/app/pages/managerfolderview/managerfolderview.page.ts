...
//import { PopoverPage } from "../modal/manager-popover/popover-component";
...
export class ManagerfolderviewPage implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((params) => {
      if (this.router.getCurrentNavigation().extras.state) {
        this.foldername = this.router.getCurrentNavigation().extras.state.folderobj;
        this.getfilelist();
      }
    });
  ...
  async getfilelist() {
    await this.api
      .getallfilelistinsidedirectorybyapptype("managerapp", this.foldername)
      .subscribe(
        (res) => {
          let value: any = [];
          if (res.length <= 0) {
  ...
  deleteFile(link) {
    var _self = this;
    const file_path = link.stored_url.substr(
      0,
      link.stored_url.lastIndexOf("/")
    );
    const file_name = link.stored_url.substr(
      link.stored_url.lastIndexOf("/") + 1
    );
    this.file.removeFile(file_path, file_name).then((msg) => {
      if (msg) {
        const index = this.storedFiles.findIndex(
          (f) => f.Native_URL == link.stored_url
        );
        if (index >= 0) this.storedFiles.splice(index, 1);
